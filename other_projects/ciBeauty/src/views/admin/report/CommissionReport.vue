<template>
  <div class="container-fluid">
        ...
        <div class="bg-white rounded-lg shadow-md p-4">
          ...
          <div class="grid grid-cols-12 gap-4">
            <div class="col-span-3">
              ...
              <div class="invalid-feedback" :state="!errorFromDate">
            ...
            <div class="col-span-3">
              ...
              <input
                ...
                maxlength="10"
                @keyup="inputDateOnly($event.target)">
              <div class="invalid-feedback" :state="!errorToDate">
                ...
                <div class="col-span-full">
                  ...
                  >
                    ...
                    <tr>
                      ...
                      <th>Ngày</th>
                      <th>Số Bill</th>
                      <th style="width: 150px !important;">Tên dịch vụ</th>
                      <th>Đơn giá</th>
                      <th>Số lượng</th>
                      <th>Tiền dịch vụ</th>
                      <th>Khuyến mãi</th>
                    ...
                    <tbody>
                      <tr>
                        <td class="total text-center font-weight-bold" colspan="5">
                          <span class="text-header">Tổng</span>
                        </td>
                        ...
                        <td class="text-right font-weight-bold text-header"></td>
                        <td class="text-right font-weight-bold text-header">{{ total_commission_amount | format_currency }}</td>
                        <td class="text-right font-weight-bold text-header">{{ total_tour_amount | format_currency }}</td>
                      </tr>
                      <tr v-for="(commission, index) in commissions" :key="index">
                        ...
                        <td class="text-right">
                          ...
                          <i class="fa fa-edit" title="Sửa tiền hoa hồng"
                             @click="showModalEditCommission(commission, 'Tiền hoa hồng', 1)"/>
                        </td>
                        <td class="text-right">
                          {{ commission.tour_amount | format_currency }}
                          <i class="fa fa-edit" title="Sửa tiền tour"
                             @click="showModalEditCommission(commission, 'Tiền tour', 2)"/>
                        </td>
    ...
    <!-- Modal edit commission-->
    <div v-if="showUpdateCommissionModal" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4">
        <div class="flex items-center justify-between p-4 border-b">
          <h5 class="text-xl font-bold">Cập nhật tiền tour/tiền hoa hồng</h5>
          <button @click="showUpdateCommissionModal = false" class="text-gray-500 hover:text-gray-700">
        ...
        <div class="p-6">
          <div class="grid grid-cols-12 gap-4">
            <div class="col-span-full">
              <label>Hóa đơn: {{currentCommission.bill_number}}</label>
          </div>
          </div>

          <div class="grid grid-cols-12 gap-4">
            <div class="col-span-full">
              <label>Nhân viên: {{currentCommission.staff_name}}</label>
          </div>
          </div>

          <div class="grid grid-cols-12 gap-4">
            <div class="col-span-full">
              <label>{{currentCommission.title}} hiện tại:
                {{currentCommission.amount_old | format_currency}}
              </label>
          </div>
          </div>
          ...
          <div class="grid grid-cols-12 gap-4">
            <div class="col-span-full">
              <label> Sửa thành<span class="error-sybol"></span></label>
                  <input
                  id="new_amount"
                  type="text"
                  autocomplete="new-password"
                  class="form-control"
                  v-model="currentCommission.amount_new"
          ...
          <div class="grid grid-cols-12 gap-4">
            <div class="col-span-full">
              <label> Lý do sửa<span class="error-sybol"></span></label>
              <textarea class="form-control"
                rows="3"
                v-model="currentCommission.note">
              </textarea>
          </div>
          </div>
          ...
          <div class="grid grid-cols-12 gap-4 mt-4">
            <div class="col-span-4 text-left mt-3">
              <button class="btn btn-outline-danger pull-left btn-width-120" @click="cancelEditCommission()">
                Hủy
              </button>
            </div>
            <div class="col-span-8 text-right mt-3">
              <span class="loading-more" v-show="loadingConfirmEdit"><icon name="loading" width="60"/></span>
              <button class="btn btn-outline-success float-right btn-width-120"  v-show="!loadingConfirmEdit"
                        @click="confirmEditCommission"
                        :disabled="saving">
                Xác nhận
              </button>
            </div>
...
export default {
  ...
  methods: {
      ...
      adminAPI
        ...
        .then(res => {
          if (res && res.data && res.data.data) {
            ...
            this.total_tour_amount = res.data.data.total_tour_amount
    ...
    getStaffOption() {
      ...
      showModalEditCommission(commission, title, type) {
        this.currentCommission = commission
        this.currentCommission.title = title
        this.currentCommission.type = type
          if(type == 1) {
              this.currentCommission.amount_old = commission.commission_amount
          } else {
              this.currentCommission.amount_old = commission.tour_amount
          }
      },
    ...
    confirmEditCommission() {
        if(!this.currentCommission.amount_new) {
            if(this.currentCommission.amount_new != 0) {
                this.popToast('danger', "Vui lòng nhập [Sửa thành]!")
                return
            }
        }
        if(!this.currentCommission.note || !this.currentCommission.note.trim()) {
            this.popToast('danger', "Vui lòng nhập [lý do sửa]!")
            return
        }
      this.loadingConfirmEdit = true
      let params = {
        "id": this.currentCommission.id,
        "type": this.currentCommission.type,
        "amount_old": this.currentCommission.amount_old,
        "amount_new": this.currentCommission.amount_new,
        "note": this.currentCommission.note
      }
      adminAPI
        .editCommissionReport(params)
        .then(res => {
