    ...
    <b-row>
      <b-col>
        <b-card>
          ...
          <b-row>
            <b-col md='12'>
              <h4 class="mt-2 text-center text-header">Danh Sách Tạm Ứng</h4>
            </b-col>
          </b-row>
          ...
          <b-row>
            ...
            <b-col md="2">
              <label v-show="time_option != 4 && time_option != 5"> {{time_option == 1 ? 'Đến ngày' : time_option == 2 ? 'Tháng' : 'Quý' }} </label>
          ...
          <b-row class="mt-2 mb-2">
            <b-col md="12">
              <b-button variant="outline-primary" class="pull-right btn-width-120"
                        :disabled="onSearch" @click="prepareToSearch">
                Tìm Kiếm
              </b-button>
            </b-col>
          </b-row>
            ...
            <b-col class="table-cus">
              <table class="table table-bordered table-striped fixed_header">
                  ...
                  <tr>
                    ...
                    <th style="width:9%" class="text-center">Số điện thoại</th>
                    <th style="width:9%" class="text-center">Lý do tạm ứng</th>
                    <th style="width:9%" class="text-center">Trạng thái</th>
                    <th style="width:9%" class="text-center">Ngày mượn</th>
                    <th style="width:9%" class="text-center">Ngày hẹn trả</th>
                    <th style="width:9%" class="text-center">Số tiền</th>
                    <th style="width:9%" class="text-center">Đã trả</th>
  ...
  methods: {
    ...
    getOptions() {
      ...
      loanApi.getOptionsRelated(params).then(res => {
        if(res != null && res.data != null && res.data.data != null) {
          ...
          let itemEmpty = {"value": null, "text": '-- Tất cả --'}
          this.staffOptions.unshift(itemEmpty)
        }
    ...
    search() {
      ...
      let params = {
        ...
        "from_date": fromDate,
        "to_date": toDate,
        "staff_id": this.staffSelect.value ? this.staffSelect.value : null,
     ...
     */
    edit (id) {
        this.$router.push('/advance/edit/' + id)
    },

    deleted (id, name) {
      if(id && name) {
        this.$bvModal.msgBoxConfirm('Xóa tạm ứng của [' + name + "]. Bạn có chắc không?", {
          title: false,
          buttonSize: 'sm',
