<template>
  <div class="container-fluid px-4 py-6">
    <div class="bg-white rounded-lg shadow p-6">
      ...
      <!-- Title -->
      <h4 class="text-2xl font-semibold text-center text-orange-600 mb-4">Danh Sách Tạm Ứng</h4>
      <hr class="mb-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
        ...
        <div>
          <label v-show="time_option != 5" class="block text-sm font-medium text-gray-700 mb-1">{{time_option == 1 ? 'Từ ngày' : 'Năm'}}</label>
          <datepicker
        ...
        <div>
          <label v-show="time_option != 4 && time_option != 5" class="block text-sm font-medium text-gray-700 mb-1">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full border border-gray-300">
          <thead class="bg-gray-50">
            <tr>
              <th class="px-4 py-2 text-center text-xs font-medium text-gray-700 uppercase border" style="width:5%">STT</th>
              <th class="px-4 py-2 text-center text-xs font-medium text-gray-700 uppercase border" style="width:18%">Tên NV</th>
              <th class="px-4 py-2 text-center text-xs font-medium text-gray-700 uppercase border" style="width:9%">Số điện thoại</th>
              <th class="px-4 py-2 text-center text-xs font-medium text-gray-700 uppercase border" style="width:9%">Lý do tạm ứng</th>
              <th class="px-4 py-2 text-center text-xs font-medium text-gray-700 uppercase border" style="width:9%">Trạng thái</th>
              <th class="px-4 py-2 text-center text-xs font-medium text-gray-700 uppercase border" style="width:9%">Ngày mượn</th>
              <th class="px-4 py-2 text-center text-xs font-medium text-gray-700 uppercase border" style="width:9%">Ngày hẹn trả</th>
              <th class="px-4 py-2 text-center text-xs font-medium text-gray-700 uppercase border" style="width:9%">Số tiền</th>
              <th class="px-4 py-2 text-center text-xs font-medium text-gray-700 uppercase border" style="width:9%">Đã trả</th>
              <th class="px-4 py-2 text-center text-xs font-medium text-gray-700 uppercase border" style="width:9%">Còn lại</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-2 text-sm border">{{item.staff_name}}</td>
              <td class="px-4 py-2 text-sm border">{{item.staff_phone}}</td>
              <td class="px-4 py-2 text-sm border">{{item.advance_reason}}</td>
              ...
              <td class="px-4 py-2 text-sm border">
                <div class="flex flex-col gap-2">
                  <div v-if="item.status == -1 || isRoot" class="flex gap-2">
                    ...
                    <i class="fa fa-trash cursor-pointer text-red-600 hover:text-red-800" title="Xóa" @click="deleted(item.id, item.staff_name)"></i>
                  </div>
                  <button
                    v-if="item.status != 2"
...
const excel_fields = reactive({
  'Tên NV': 'staff_name',
  'Số điện thoại': 'staff_phone',
  'Lý do tạm ứng': 'advance_reason',
...
function getOptions() {
  ...
  loanApi.getOptionsRelated(params).then(res => {
    if (res != null && res.data != null && res.data.data != null) {
      ...
      const itemEmpty = {"value": null, "text": '-- Tất cả --'}
      staffOptions.value.unshift(itemEmpty)
    }
...
function search() {
  ...
  const params = {
    ...
    "from_date": fromDate,
    "to_date": toDate,
    "staff_id": staffSelect.value.value ? staffSelect.value.value : null,
    "status": inputs.status,
    "limit": pageLimit.value,
    "offset": offset.value
  }

  loanApi.searchAdvance(params).then(res => {
    if (res != null && res.data != null && res.data.data != null) {
...
function goToPayment(id) {
  window.open('/advance-payment/' + id, '_blank')
}
...
function getListPayment(id) {
  ...
  loanApi.getListPayment({"id": id, "type": 0}).then(res => {
