<template>
  <div class="w-full mx-auto px-4">
    <div class="bg-white rounded-lg shadow">
      <div class="p-6">
        <h4 class="text-xl font-semibold text-center text-orange-600 mb-4">Thanh Toán Khoản Đi Vay</h4>
        <hr class="mb-6" />
...
const getLoanDetail = async () => {
  ...
  if (loanId) {
    await loanApi.getLoanDetail(loanId)
      .then((result) => {
        if (result?.data?.data) {
    ...
    .then((result) => {
      if (result?.data?.data) {
        ...
        if (options.expend_income_groups) {
          ...
          const expend_group_code = 'khoan_di_vay'
          for (const item of options.expend_income_groups) {
...
const save = async () => {
  ...
  // Add payment
  saving.value = true
  await loanApi.addLoanInPayment(data)
    .then(() => {
  ...
  if (inputs.expend_income_group_id) {
    ...
    for (const item of expendTypeOptionStore.value) {
      if (item.group_id == inputs.expend_income_group_id) {
        ...
        if (item.name == 'Chi trả tiền đi vay') {
