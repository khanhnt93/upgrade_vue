<template>
  <div class="container-fluid px-4 py-6">
    <div class="bg-white rounded-lg shadow p-6">
      <!-- Title -->
      <h4 class="text-2xl font-semibold text-center text-orange-600 mb-4">{{title_prefix}} Khoản Đi Vay</h4>
      <hr class="mb-6">
...
function save(is_draft) {
  ...
  saving.value = true
  if (inputs.id) {
    loanApi.updateLoanIn(submitData).then(res => {
      saving.value = false
    ...
    loanApi.addLoanIn(submitData).then(res => {
      saving.value = false
      if (res != null && res.data != null) {
        if (res.data.status == 200) {
          router.push("/loan-in")
