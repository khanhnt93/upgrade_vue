...
const exportExcel = () => {
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'Danh sách khoản cho vay')
  XLSX.writeFile(wb, 'Danh sách khoản cho vay.xls', { bookType: 'xls' })
}
...
function edit(id) {
  router.push('/loan-out/edit/' + id)
}

async function deleted(id, name) {
  if (id && name) {
    if (await confirmDialog('Xóa khoản vay của [' + name + "]. Bạn có chắc không?")) {
...
function goToPayment(id) {
  window.open('/loan-out-payment/' + id, '_blank')
}
...
function getListPayment(id) {
  ...
  loanApi.getListPayment({"id": id, "type": 2}).then(res => {
