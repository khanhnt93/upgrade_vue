  ...
  <div class="w-full px-4 py-6">
    <div class="bg-white rounded-lg shadow">
      <div class="p-6">
        ...
        <h4 class="text-2xl font-semibold text-center mb-4 text-header">Danh Sách Nhóm Sản Phẩm</h4>
        <hr class="mb-6">
  ...
  if (await confirmDialog(`Xóa ${name}. Bạn có chắc không?`)) {
    productApi.deleteProductGroup(id).then(res => {
      if(res != null && res.data != null) {
        popToast('success', "Xoá nhóm sản phẩm thành công!")
        getProductGroupList()
      }
    }).catch(err => {
      // Handle error
...
const edit = (id) => {
  router.push('/product-group/index/' + id)
}

const goToAdd = () => {
  router.push('/product-group/index/')
}
