<template>
  <div class="w-full mx-auto px-4">
    <div class="bg-white rounded-lg shadow p-6">
      <div class="mb-4">
        <h4 class="text-xl font-semibold text-center text-orange-600 mb-4">{{title_prefix}} Khoản Chi</h4>
      </div>
      <hr class="my-4">
      ...
      <div class="grid grid-cols-12 gap-4 mb-4">
        <div class="col-span-12 md:col-span-3">
          <label class="block mb-2 font-medium">Đối tượng chi</label>
        </div>
        <div class="col-span-12 md:col-span-9">
      ...
      <div class="grid grid-cols-12 gap-4 mb-4">
        <div class="col-span-12 md:col-span-3">
          <label class="block mb-2 font-medium">Sổ khoản chi<span class="text-red-500">*</span></label>
        </div>
        <div class="col-span-12 md:col-span-9">
      ...
      <div class="grid grid-cols-12 gap-4 mb-4">
        <div class="col-span-12 md:col-span-3">
          <label class="block mb-2 font-medium">Nhóm khoản chi<span class="text-red-500">*</span></label>
        </div>
        <div class="col-span-12 md:col-span-9">
          <select v-model="inputs.expend_income_group_id"
                  @change="changeExpendGroup"
                  class="form-select w-full rounded-md border-gray-300">
            <option v-for="opt in expendGroupOptions" :key="opt.value" :value="opt.value">{{opt.text}}</option>
          </select>
        </div>
      </div>
      ...
      <div class="grid grid-cols-12 gap-4 mb-4">
        <div class="col-span-12 md:col-span-3">
          <label class="block mb-2 font-medium">Loại khoản chi<span class="text-red-500">*</span></label>
        </div>
        <div class="col-span-12 md:col-span-9">
          <select v-model="inputs.expend_income_type_id"
                  ...
                  class="form-select w-full rounded-md border-gray-300">
            <option v-for="opt in expendTypeOptions" :key="opt.value" :value="opt.value">{{opt.text}}</option>
          </select>
        </div>
      </div>
  ...
  if(fundHisId) {
    ...
    fundApi.getFundHisDetail(fundHisId).then(res => {
      if(res != null && res.data != null && res.data.data != null){
        ...
        if(inputs.value.expend_income_group_id && expendTypeOptions.value.length > 0) {
          changeExpendGroup()
        }
      }
    }).catch(err => {
  ...
  fundApi.getOptionsRelatedFund(params).then(res => {
    if(res != null && res.data != null && res.data.data != null) {
      ...
      for(let item of expend_groups) {
        ...
        expendGroupOptions.value.push(option)
      }
...
const updateDescription = () => {
  ...
  if(inputs.value.expend_income_type_id) {
    expend_type_text = commonFunc.getTextByValueOfSelectedBox(expendTypeOptions.value, inputs.value.expend_income_type_id)
  }
  let order_name = ""
  if(orderBuySelect.value && orderBuySelect.value.id) {
    ...
    if (orderSellSelect.value && orderSellSelect.value.id) {
      ...
      if (customerSelect.value && customerSelect.value.text) {
        order_name += " - " + customerSelect.value.text
      }
    }
  }

  if(inputs.value.description && !inputs.value.description.includes(expend_type_text) && !inputs.value.description.includes(order_name) ) {
    return
  }
...
const save = () => {
  if(inputs.value.money_type == 0 || inputs.value.money_type == 2) {
    ...
    if(!inputs.value.bank_account_id) {
      popToast('danger', "[Sổ khoản chi] là mục bắt buộc nhập")
      return
    }
  }

  if(!inputs.value.expend_income_group_id) {
    popToast('danger', "[Nhóm khoản chi] là mục bắt buộc nhập")
    return
  }
  if(!inputs.value.expend_income_type_id) {
    popToast('danger', "[Loại Khoản chi] là mục bắt buộc nhập")
    return
  }
...
const changeExpendGroup = () => {
  if(inputs.value.expend_income_group_id) {
    ...
    for(let item of expendTypeOptionStore.value) {
      if(item.type == 1 && item.group_id == inputs.value.expend_income_group_id) {
