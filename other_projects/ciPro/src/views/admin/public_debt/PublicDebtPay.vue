  ...
  <div class="container mx-auto px-4">
    <div class="bg-white rounded-lg shadow">
      <div class="p-6">
        ...
        <div class="flex mb-6">
          <div class="w-1/4">
            <label class="block mt-2 font-semibold">Nhà cung cấp<span class="text-red-500"></span></label>
          </div>
          <div class="w-3/4">
            <multiselect
            ...
            <table class="min-w-full border-collapse border border-gray-300">
              <thead class="bg-gray-100">
                <tr>
                  ...
                  <th class="border border-gray-300 px-4 py-2 text-center">Số ĐH nhập</th>
                  <th class="border border-gray-300 px-4 py-2 text-center">Tên nhà cung cấp</th>
...
const getOptions = async () => {
  ...
  try {
    const res = await publicDebtApi.getOptionsRelatedPublicDebt({ 'supplier-all': true })
    if (res?.data?.data) {
      // Supplier
      ...
      if (supplier_id) {
        for (const supplier of supplierOptions.value) {
          if (supplier.id == supplier_id) {
            supplierSelect.value = supplier
            break
          }
        }
        debt.supplier_id = supplier_id
...
const changeSupplier = () => {
  if (supplierSelect.value.id) {
    debt.supplier_id = supplierSelect.value.id
    // Get lại danh sách các khoản vay
    getListPublicDebtBySupplier()
  } else {
    debt.supplier_id = null
  }
}
  ...
  for (const item of debtList.value) {
    if (amount_pay_remaining <= 0) {
      ...
      if (amount_pay_remaining > parseInt(item.amount_remaining)) {
        item.total = currencyFormat(JSON.parse(JSON.stringify(item.amount_remaining)))
...
const getListPublicDebtBySupplier = async () => {
  if (!debt.supplier_id) return
  if (loading.value) return

  loading.value = true
  try {
    const res = await publicDebtApi.getPublicDebtBySupplier(debt.supplier_id)
    if (res?.data?.data) {
