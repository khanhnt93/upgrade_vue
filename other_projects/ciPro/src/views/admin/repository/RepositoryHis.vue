<template>
  <div class="w-full px-4 py-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
        ...
        <div>
          ...
          <select
            ...
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            <option v-for="option in productBrandOptions" :key="option.value" :value="option.value">{{option.text}}</option>
          </select>
        </div>
        ...
        <table class="min-w-full border-collapse border border-gray-300">
          <thead class="bg-gray-100">
            <tr>
              ...
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border" style="width:8%">Loại hoạt động</th>
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border" style="width:8%">Số phiếu</th>
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border" style="width:8%">Mã hàng</th>
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border" style="width:13%">Tên hàng</th>
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border" style="width:10%">Hãng</th>
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border" style="width:6%">Đơn vị</th>
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border" style="width:8%">Số lượng</th>
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border" style="width:9%">Đơn giá</th>
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border" style="width:9%">Thành tiền</th>
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border" style="width:8%">Người thao tác</th>
              <th class="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase border" style="width:3%"></th>
            </tr>
          </thead>
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-2 text-sm border">{{item.type_str}}</td>
              <td class="px-4 py-2 text-sm text-center border">
                <p>{{item.repository_number}}</p>
                <button
                  v-show="!creatingPHG"
                  v-if="item.type == 1 && item.order_sell_id"
                  @click="preparePrintPHG(item)"
                  class="px-4 py-1 bg-white text-green-600 border border-green-600 rounded hover:bg-green-50 transition-colors text-xs mt-1"
                >
                  In phiếu xuất kho
                </button>
              ...
              <td class="px-4 py-2 text-sm border">{{item.unit_name}}</td>
              <td class="px-4 py-2 text-sm border">{{currencyFormat(item.quantity)}}</td>
              <td class="px-4 py-2 text-sm border">{{currencyFormat(item.price)}}</td>
              ...
              <td class="px-4 py-2 text-sm border">
                <button v-if="isUserRoot" @click="deleted(item.id, item.repository_number)" class="btn btn-sm btn-danger text-xs"><i class="fa fa-trash"></i></button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <!-- Print Area (Hidden) -->
    <div hidden id="contentPrintPHG" class="p-6">
...
const exportToExcel = () => {
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, 'Lịch sử kho')
  XLSX.writeFile(workbook, 'lich_su_kho.xlsx')
}
...
function getOptionRelatedRepository() {
  ...
  repositoryApi.getOptionsRelatedRepository(params).then(res => {
    if (res != null && res.data != null && res.data.data != null) {
      ...
      const product_brands = options.product_brands
...
function changeProductGroup() {
  if (productGroupSelect.value && productGroupSelect.value.id) {
    ...
    const itemTypeEmpty = {"id": null, "name": "--Tất cả--", "product_group_id": null}
...
async function preparePrintPHG(repoHis) {
...
async function getRepositoryDetailForPrint(id) {
...
function search() {
  ...
  repositoryApi.getRepositoryHis(params).then(res => {
    if (res != null && res.data != null && res.data.data != null) {
      const it = res.data.data.repository_historys
...
function goToAddOutput() {
  router.push("/repo-output")
}

function goToAddInput() {
