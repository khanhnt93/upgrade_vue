  ...
  <div class="container-fluid px-4 py-4">
    <div class="bg-white rounded-lg shadow">
      <div class="p-6">
        ...
        <!-- Title -->
        <h4 class="text-xl font-semibold text-center mb-4">Ghi Nợ</h4>
        <hr class="mb-4">
        ...
        <div class="space-y-4">
          ...
          <!-- Bill Number -->
          <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
            <label class="md:col-span-3 pt-2 text-sm font-medium text-gray-700">Số bill</label>
            <div class="md:col-span-9">
              <input
                ...
                maxlength="100"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                :class="{ 'border-red-500': errorBillNumber }">
              <p v-if="errorBillNumber" class="text-red-500 text-sm mt-1">Số bill không đúng!</p>
            </div>
          </div>
...
const billNumberNotExists = ref(false)
...
const errorBillNumber = computed(() => {
  return billNumberNotExists.value
})
...
const save = () => {
  ...
  if (id) {
    // Edit
    debt.value.id = id
    adminAPI.updateDept(debt.value).then(res => {
      saving.value = false
      ...
      if (err.response.data.status == 422) {
        billNumberNotExists.value = true
      } else {
        const errorMess = commonFunc.handleStaffError(err)
        popToast('danger', errorMess)
      }
    ...
    // Add
    adminAPI.addDept(debt.value).then(res => {
      saving.value = false
      if (res != null && res.data != null) {
        if (res.data.status == 200) {
          router.push('/debt/list')
        }
      }
    }).catch(err => {
      saving.value = false
