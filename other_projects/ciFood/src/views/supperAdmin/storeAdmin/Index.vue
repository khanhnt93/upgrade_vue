    ...
    <div class="bg-white rounded-lg shadow">
      <div class="p-6">
        ...
        <div class="space-y-4">
          ...
          <!-- Store -->
          <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
            <label class="md:col-span-3 pt-2 text-sm font-medium text-gray-700">Cửa Hàng</label>
            <div class="md:col-span-9">
              <select
...
// Computed
const errorName = computed(() => checkInfo(adminStore.value.name))
const errorPhone = computed(() => checkInfo(adminStore.value.phone_number))

const errorPassword = computed(() => {
  if (!route.params.id) {
    return checkInfo(adminStore.value.password)
  }
  return false
})

const errorLengthPassword = computed(() => {
  if (errorPassword.value || !adminStore.value.password)
    return false
  if (saving.value)
    return (adminStore.value.password.length < 6)
  return false
})
...
const getAdminStoreDetail = () => {
  ...
  if (AdminStoreId) {
    superAdminAPI.getAdminStoreDetail(AdminStoreId).then(res => {
      if (res != null && res.data != null && res.data.data != null) {
        adminStore.value = res.data.data
      }
    }).catch(err => {
      const errorMess = commonFunc.handleStaffError(err)
      popToast('danger', errorMess)
...
const save = () => {
  ...
  if (result) {
    ...
    if (adminStoreId) {
      // Edit
      const adminStoreData = adminStore.value
      ...
      // Add
      superAdminAPI.addAdminStore(adminStore.value).then(res => {
        saving.value = false
        if (res != null && res.data != null) {
          if (res.data.status == 200) {
...
const changeBrand = () => {
  const brandId = adminStore.value.brand_id
  if (brandId) {
    adminStore.value.store_id = null
  }
}

const changeStore = () => {
  const storeId = adminStore.value.store_id
  if (storeId) {
    adminStore.value.brand_id = null
  }
}
