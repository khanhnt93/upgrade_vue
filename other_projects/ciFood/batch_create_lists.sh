...
# Create RoleList
cat > "src/views/admin/role/RoleList_new.vue" << 'EOF'
<template>
  <div class="container-fluid px-4 py-6">
    <div class="bg-white rounded-lg shadow-lg p-6">
      ...
      <h4 class="text-2xl font-bold text-center text-gray-800 mb-4">Danh Sách Nhóm Quyền</h4>
      <hr class="mb-6" />
        ...
        <table class="min-w-full border border-gray-300">
          <thead class="bg-gray-100">
            <tr>
              ...
              <th class="px-4 py-2 border-b text-left">Mã nhóm quyền</th>
              <th class="px-4 py-2 border-b text-left">Tên nhóm quyền</th>
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-4 py-2 border-b">{{ item.code }}</td>
              ...
              <td class="px-4 py-2 border-b">
                <div class="flex justify-center gap-2">
                  ...
                  <button @click="deleted(item.id, item.name)" class="text-danger-500 hover:text-danger-700 p-2" title="Xóa">
...
echo "RoleList created"

# Create GroupTableList
cat > "src/views/admin/group_table/GroupTableList_new.vue" << 'EOF'
<template>
  <div class="container-fluid px-4 py-6">
    <div class="bg-white rounded-lg shadow-lg p-6">
      <div class="flex justify-end mb-4">
      ...
      <h4 class="text-2xl font-bold text-center text-gray-800 mb-4">Danh Sách Nhóm Bàn</h4>
      <hr class="mb-6" />
      <div class="overflow-x-auto">
        <table class="min-w-full border border-gray-300">
          <thead class="bg-gray-100">
...
const deleted = (id, name, rowIndex) => {
  if (confirm(`Xóa [${name}]. Bạn có chắc không?`)) {
    adminAPI.deleteGroupTable(id).then(res => {
      ...
      toast.success('Xóa nhóm bàn thành công!!!')
    }).catch(err => {
      let errorMess = commonFunc.handleStaffError(err)
      toast.error(errorMess)
    })
...
const edit = (id) => {
  router.push('/group-table/edit/' + id)
}
...
echo "GroupTableList created"

# Create KitchenAreaList
cat > "src/views/admin/kitchen_area/KitchenAreaList_new.vue" << 'EOF'
<template>
  <div class="container-fluid px-4 py-6">
    <div class="bg-white rounded-lg shadow-lg p-6">
      <div class="flex justify-end mb-4">
...
echo "KitchenAreaList created"
echo "All list components created successfully!"
