            ...
            <div class="print-list">
              ...
              <div class="content" style="border-bottom: solid 1px black; border-top: solid 1px black;">
                ...
                <div v-if="services.data" v-for="item in services.data"
                  ...
                  <div v-if="item.type === 'payment'" class="border-gray-bottom currency-block" >
                  ...
                  <div v-if="item.type === 'service'" style="flex: 1; padding-top: 10px" v-bind:class="{ 'border-gray-top': item.index === 0 }">
                    <span v-if="item.index === 0">Dịch vụ, phụ thu</span></div>
                  <div v-if="item.type === 'service'" style="display: flex; flex-direction: column; flex: 3;" v-bind:class="{ 'border-gray-top': item.index === 0 }">
                    <div
                      style="display: flex; flex-direction: row; justify-content: space-between; border-bottom: solid 1px gray; padding: 10px 0;">
                      {{ item.data.created_at + ': ' + item.data.name }}
                    </div>
                  </div>
...
export default {
  ...
  methods: {
    ...
    getPaymentInfo() {
      ...
      adminAPI.getPaymentInfoForPrint(paymentId).then(res => {
    ...
    goBack() {
      ...
      if (fromWhere == "payment") {
        this.$router.push('/booking/payment/' + paymentId)
      } else {
        this.$router.push('/total-order-history')
      }
