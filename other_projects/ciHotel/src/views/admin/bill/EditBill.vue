<template>
  <div class="container-fluid">
        ...
        <b-card>
          <b-row>
            <b-col cols="6" class="pt-2">
              <b-button variant="secondary" class="pull-left px-4 fix-width-btn-200" @click="goBack()">
                Quay lại
              </b-button>
            </b-col>
          ...
          <b-row>
            <b-col cols="12" class="pt-2">
              <button class="btn btn-primary pull-right px-4 default-btn-bg fix-width-btn-200"
                      @click="getPaymentInfoForEditBill()" :disabled="paymentInfo.length == 0">
                Làm mới thông tin
              </button>
            </b-col>
          </b-row>
          ...
          <b-row>
            <b-col class="text-center">
              <h5 class="text-header mt-3">Thông Tin Thanh Toán</h5>
            </b-col>
          </b-row>
          ...
          <b-row class="mt-3 pb-3">
            ...
            <b-col md="6">
              ...
              <p class="col-12 mt-3"><b>Khách hàng: </b>{{this.paymentInfo.customer_name}}
                <i class="fa fa-edit" style="margin-left:5px" @click="$bvModal.show('modal-edit-customer-name')"></i>
              </p>
              <p class="col-12 mt-3">
                <b>Số lượng khách: </b>
              ...
              <p class="col-12 mt-3"><b>Booking giờ đến:</b> {{this.paymentInfo.time_in}}<i class="fa fa-edit" style="margin-left:5px" @click="$bvModal.show('modal-edit-time-in')"></i></p>
              <p class="col-12 mt-3"><b>Booking giờ đi:</b> {{this.paymentInfo.time_out}}<i class="fa fa-edit" style="margin-left:5px" @click="$bvModal.show('modal-edit-time-out')"></i></p>
              <p class="col-12 mt-3"><b>Giờ khách đến thực tế:</b> {{this.formattedActualCheckIn}}<i class="fa fa-edit" style="margin-left:5px" @click="$bvModal.show('modal-edit-actual-checkin')"></i></p>
            ...
            <b-col md="6">
              <h4 class="col-12 mt-3"><b>Thành tiền: </b>
                <p class="pull-right">
                  <i class="fa fa-edit" @click="$bvModal.show('modal-edit-total')"></i>
                  ...
                  {{currencyFormat(this.paymentInfo.total)}}
                </p>
              </h4>
              ...
              <p class="col-12 mt-2" v-if="this.paymentInfo.vat_percent">
                <b>Thuế VAT: </b>
                <input class="largerCheckbox mr-2" type="checkbox" name="open" v-model="paymentInfo.apply_vat"
                       @change="applyVat" :disabled="isPayment"/>
                <input
                    id="vat_percent"
                    type="text"
                ...
                <b-col>
                  ...
                  <b-row class="form-row ml-3">
                    <b-col>
                      ...
                      <p>E_money: {{currencyFormat(this.paymentInfo.e_money)}}</p>
                    </b-col>
                  </b-row>
                  ...
                  <b-row class="form-row">
                    <b-col md="12" class="mt-2">
                      <div class="input-group">
                        ...
                        <b-button class="width-icon ml-2 btn-sm" variant="secondary" type="button" @click="showModalApplyPmt" :disabled="isPayment">
                          <i class="fa fa-plus "/>
                        </b-button>
                      </div>
                    </b-col>
                ...
                <b-col>
                  ...
                  <b-row class="form-row ml-3" v-show="this.paymentInfo.service && this.paymentInfo.service.length > 0">
                    <b-col md="12" class="mt-2">
                      ...
                      <p v-for="(service, index) in this.paymentInfo.service" :key="service.name + index">
                        {{service.quantity + " x " + service.name + " (" + currencyFormat(service.price) + ") = " + currencyFormat(service.price * service.quantity) }}
                        <i class="fa fa-trash" @click="deleteService(service.name, service.price)"/>
                      </p>
                    </b-col>
                  </b-row>
              ...
              <p class="col-12 mt-3"><b>Chi tiết thanh toán: </b></p>
    ...
    <!-- Modal edit total -->
    <b-modal centered hide-footer hide-header id="modal-edit-total">
      <b-row class="form-row">
        <b-col md="12" >
          <span> Nhập thành tiền </span><span class="error-sybol"></span>
          <input
            ref="total"
            type="number"
            :value="this.paymentInfo.total"
            autocomplete="new-password"
            class="form-control"
          >
        </b-col>
      ...
      <b-row class="d-flex justify-content-center mt-3">
        <button class="btn btn-primary px-4 default-btn-bg" style="margin-right: 5px" @click="confirmEditTotal">
          Xác nhận
        </button>
        <b-button @click="$bvModal.hide('modal-edit-total')">
          Hủy bỏ
        </b-button>
      </b-row>
    </b-modal>
    ...
    <b-modal centered hide-footer hide-header id="modal-edit-customer-name">
      <b-row class="form-row">
        <b-col md="12" >
          <span> Tên khách hàng </span><span class="error-sybol"></span>
          <input
            id="customer_name"
            ref="customer_name"
            type="text"
            :value="this.paymentInfo.customer_name"
            autocomplete="new-password"
            class="form-control"
          >
        </b-col>
      ...
      <b-row class="d-flex justify-content-center mt-3">
        <button class="btn btn-primary px-4 default-btn-bg" style="margin-right: 5px" @click="confirmEditCustomerName">
          Xác nhận
        </button>
        <b-button @click="$bvModal.hide('modal-edit-customer-name')">
          Hủy bỏ
        </b-button>
      </b-row>
    </b-modal>
    ...
    <!-- Modal edit time in -->
    <b-modal centered hide-footer hide-header id="modal-edit-time-in">
      <b-row>
        <b-col>
          <h5 class="text-center">Nhập booking giờ đến:</h5>
        </b-col>
      </b-row>
      <hr>
      <b-row class="form-row" style="margin-bottom: 5px">
        <b-col md="12">
          ...
          <input
            ...
            id="time-in"
            type="text"
            v-model="this.paymentInfo.time_in"
            :readonly="true"
          />
        </b-col>
      </b-row>
      <b-row class="form-row" style="margin-bottom: 5px">
        <b-col md="12">
          ...
          <Datepicker
            ref="editTimeIn"
            :value="initDate(this.paymentInfo.time_in)"
            format="DD-MM-YYYY H:i:s"
          />
        </b-col>
      </b-row>
      <b-row class="d-flex justify-content-center">
        <button class="btn btn-primary px-4 default-btn-bg" style="margin-right: 5px" @click="confirmEditTimeIn">
          Xác nhận
        </button>
        <b-button @click="$bvModal.hide('modal-edit-time-in')">
          Hủy bỏ
        </b-button>
      </b-row>
    ...
    <!-- Modal edit time in -->
    <b-modal centered hide-footer hide-header id="modal-edit-time-out">
      <b-row>
        <b-col>
          <h5 class="text-center">Nhập booking giờ đi:</h5>
        </b-col>
      </b-row>
      <hr>
      <b-row class="form-row" style="margin-bottom: 5px">
        <b-col md="12">
          ...
          <input
            ...
            id="time-out"
            type="text"
            v-model="this.paymentInfo.time_out"
            :readonly="true"
          />
        </b-col>
      </b-row>
      <b-row class="form-row" style="margin-bottom: 5px">
        <b-col md="12">
          <span> Thời gian mới:</span>
          <Datepicker
            :value="initDate(this.paymentInfo.time_out)"
            ref="editTimeOut"
            format="DD-MM-YYYY H:i:s"
          />
        </b-col>
      ...
      <b-row class="d-flex justify-content-center">
        <button class="btn btn-primary px-4 default-btn-bg" style="margin-right: 5px" @click="confirmEditTimeOut">
          Xác nhận
        </button>
        <b-button @click="$bvModal.hide('modal-edit-time-out')">
          Hủy bỏ
        </b-button>
      </b-row>
    ...
    <b-modal centered hide-footer hide-header id="modal-edit-actual-checkin">
      <b-row>
        <b-col>
          <h5 class="text-center">Nhập thời gian khách đến thực tế:</h5>
        </b-col>
      </b-row>
      <hr>
      <b-row class="form-row" style="margin-bottom: 5px">
        <b-col md="12">
          ...
          <input
            ...
            id="actual_checkin"
            type="text"
            v-model="this.formattedActualCheckIn"
            :readonly="true"
          />
        </b-col>
      </b-row>
      <b-row class="form-row" style="margin-bottom: 5px">
        <b-col md="12">
          <span> Thời gian mới:</span>
          <Datepicker
            :value="initDate(formattedActualCheckIn)"
            ref="editActualCheckIn"
            format="DD-MM-YYYY H:i:s"
          />
        </b-col>
      </b-row>
      <b-row class="d-flex justify-content-center">
        <button class="btn btn-primary px-4 default-btn-bg" style="margin-right: 5px" @click="confirmEditActualCheckIn">
          Xác nhận
        </button>
        <b-button @click="$bvModal.hide('modal-edit-actual-checkin')">
          Hủy bỏ
        </b-button>
      </b-row>
    ...
    <b-modal title="Áp dụng khuyến mãi" centered hide-header hide-footer no-close-on-backdrop id="modal-apply-pmt" size="xl">
      ...
      <b-row class="mt-2">
        ...
        <b-col cols="6">
          ...
          <button v-show="!isPayment && !loadingConfirmPmt" class="btn btn-primary pull-right px-4 default-btn-bg" @click="confirmApplyPmt()">
            Xác nhận &nbsp;
          </button>
        </b-col>
      </b-row>
...
export default {
  ...
  mounted() {
    this.storeId = this.$store.state.user.store_id;
  ...
  methods: {
    /**
     * Initial date value for date modals
     */
    initDate(time) {
    ...
    deletePromotion(id) {
      ...
      adminAPI.updatePaymentInfoForEditBill(this.paymentInfo).then(res => {
    ...
    /**
     * Confirm edit customer name
     */
    confirmEditTotal() {
      const newTotal = this.$refs.total
      if (newTotal.value == ""){
        return ;
      } else {
        this.paymentInfo.total = newTotal.value
        this.$bvModal.hide('modal-edit-total')
      }
     ...
     * Confirm edit customer name
     */
    confirmEditCustomerName() {
      const customer_name = this.$refs.customer_name
      if (customer_name.value == ""){
        this.popToast('warning','Hãy nhập tên KH')
      } else {
        this.paymentInfo.customer_name = customer_name.value
        adminAPI.updatePaymentInfoForEditBill(this.paymentInfo).then(res => {
          this.popToast('success', 'Lưu thông tin thành công');
          this.getPaymentInfoForEditBill();
          this.$bvModal.hide('modal-edit-customer-name')
        }).catch(err => {
          let errorMess = commonFunc.handleStaffError(err);
          this.popToast('danger', errorMess);
        });
    ...
    showModalApplyPmt() {
      ...
      setTimeout(this.updatePmtAllyToModal, 800)
    },
    ...
    confirmPayment() {
      // Prepare data
      // this.paymentInfo.promotionId = this.pmtApplying
    ...
    addPmtToList(pmtInfo) {
      ...
      for (let i = 0; i < this.pmtTemp.length; i++) {
        if(parseInt(this.pmtTemp[i].id) === parseInt(pmtInfo.id)) {
          this.pmtTemp[index].quantity_apply = parseInt(this.pmtTemp[i].quantity_apply) + 1
          flagCheckExist = true
          break
        }
      }
    ...
    printBill() {
      adminAPI.addEditBillHistory({
        bill_id: this.paymentInfo.payment_id,
        total: this.paymentInfo.total
      }).then(res => {
        Cookies.set(this.paymentInfo.payment_id,this.paymentInfo.total)
        Cookies.set("printFrom", "edit-bill")
        this.$router.push('/bill/print/' + this.paymentInfo.payment_id)
      }).catch(err => {
    ...
    confirmEditTimeIn(){
      if (this.$refs.editTimeIn.date) {
        this.paymentInfo.time_in = moment(this.$refs.editTimeIn.date, "DD-MM-YYYY HH:mm:ss").format("HH:mm DD-MM-YYYY");

        adminAPI.updatePaymentInfoForEditBill(this.paymentInfo).then(res => {
          this.$bvModal.hide('modal-edit-time-in');
          this.popToast('success', 'Lưu thông tin thành công');
          this.getPaymentInfoForEditBill();
        }).catch(err => {
          let errorMess = commonFunc.handleStaffError(err);
    ...
    confirmEditTimeOut(){
      if (this.$refs.editTimeOut.date) {
        this.paymentInfo.time_out = moment(this.$refs.editTimeOut.date, "DD-MM-YYYY HH:mm:ss").format("HH:mm DD-MM-YYYY");
        console.log(this.paymentInfo.time_out)

        adminAPI.updatePaymentInfoForEditBill(this.paymentInfo).then(res => {
          this.popToast('success', 'Lưu thông tin thành công');
          this.$bvModal.hide('modal-edit-time-out');
          this.getPaymentInfoForEditBill();
        }).catch(err => {
          let errorMess = commonFunc.handleStaffError(err);
          this.popToast('danger', errorMess);
    ...
    confirmEditActualCheckIn(){
      if (this.$refs.editActualCheckIn.date) {
        this.paymentInfo.actual_check_in = moment(this.$refs.editActualCheckIn.date, "DD-MM-YYYY HH:mm:ss").format("YYYY-MM-DD HH:mm:ss");

        adminAPI.updatePaymentInfoForEditBill(this.paymentInfo).then(res => {
          this.$bvModal.hide('modal-edit-actual-checkin');
          this.popToast('success', 'Lưu thông tin thành công');
          this.getPaymentInfoForEditBill();
        }).catch(err => {
          let errorMess = commonFunc.handleStaffError(err);
