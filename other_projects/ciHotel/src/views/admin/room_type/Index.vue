  ...
  <div class="container-fluid mx-auto px-4">
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      <div class="text-center mb-4">
        <h4 class="text-xl font-semibold mt-1 text-orange-500">LOẠI PHÒNG</h4>
      </div>
      <hr class="mb-4" />
  ...
  methods: {
    ...
    getRoomTypeDetail() {
      ...
      if (id) {
        ...
        adminAPI.getRoomTypeDetail(id).then(res => {
    ...
    save() {
      ...
      if (id) {
        // Edit
        this.inputs.id = id;
        adminAPI.editRoomType(this.inputs).then(res => {
          this.saving = false;
          if (res != null && res.data != null) {
            if (res.data.status == 200) {
              this.toast.success('Cập nhật loại phòng thành công!!!');
            }
          }
        }).catch(err => {
          this.saving = false;
        ...
        // Add
        adminAPI.addRoomType(this.inputs).then(res => {
          this.saving = false;
          if (res != null && res.data != null) {
            if (res.data.status == 200) {
              this.router.push('/room-type/list');
            }
          }
        }).catch(err => {
          this.saving = false;
