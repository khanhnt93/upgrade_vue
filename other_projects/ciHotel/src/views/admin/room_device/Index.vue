  ...
  <div class="container-fluid mx-auto px-4">
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      <div class="text-center mb-4">
        <h4 class="text-xl font-semibold mt-1 text-orange-500">THIẾT BỊ TRONG PHÒNG</h4>
      </div>
      <hr class="mb-4" />
      ...
      <div class="grid grid-cols-1 md:grid-cols-12 gap-4 mb-4">
        ...
        <div class="md:col-span-9">
          <input
            ...
            type="text"
            autocomplete="new-password"
            :class="['form-control w-full', errorQuantity ? 'border-red-500' : '']"
            v-model="inputs.quantity"
            maxlength="255"
          />
          <div v-if="errorQuantity" class="text-red-600 text-sm mt-1">Vui lòng nhập value</div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    errorQuantity() {
      return this.checkInfo(this.inputs.quantity);
    }
  },
  methods: {
    checkInfo(info) {
    ...
    checkValidate() {
      return !(this.errorName || this.errorQuantity);
    },
    ...
    getRoomDeviceDetail() {
      ...
      if (id) {
        ...
        adminAPI.getRoomDeviceDetail(id).then(res => {
    ...
    save() {
      ...
      if (id) {
        // Edit
        this.inputs.id = id;
        adminAPI.editRoomDevice(this.inputs).then(res => {
          this.saving = false;
        ...
        // Add
        adminAPI.addRoomDevice(this.inputs).then(res => {
          this.saving = false;
          if (res != null && res.data != null) {
            if (res.data.status == 200) {
              this.router.push('/room-device/list');
            }
          }
        }).catch(err => {
          this.saving = false;
