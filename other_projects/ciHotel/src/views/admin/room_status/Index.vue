  ...
  methods: {
    ...
    getRoomStatusDetail() {
      ...
      if (id) {
        ...
        adminAPI.getRoomStatusDetail(id).then(res => {
    ...
    save() {
      ...
      if (id) {
        // Edit
        this.inputs.id = id;
        adminAPI.editRoomStatus(this.inputs).then(res => {
          this.saving = false;
        ...
        // Add
        adminAPI.addRoomStatus(this.inputs).then(res => {
          this.saving = false;
          if (res != null && res.data != null) {
            if (res.data.status == 200) {
              this.router.push('/room-status/list');
            }
          }
        }).catch(err => {
          this.saving = false;
