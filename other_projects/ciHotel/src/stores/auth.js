...
export const useAuthStore = defineStore('auth', {
  ...
  getters: {
    ...
    isSuper: (state) => state.user?.isSuper || false,
  ...
  actions: {
    ...
    initializeAuth() {
      if (this.token) {
        session.defaults.headers['token'] = `${this.token}`
        upload.defaults.headers['token'] = `${this.token}`
      }
    }
  },

  persist: {
    enabled: false, // Using cookies and localStorage manually
  }
})
