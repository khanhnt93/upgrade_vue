    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
      <div class="w-full md:col-span-12">
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
            <div class="w-full md:col-span-12">
              <h4 class="mt-2 text-center">Danh Sách Nhóm Chi Phí</h4>
            </div>
          </div>
          <hr>
...
export default {
   ...
   */
    ...
    getExpendGroupList () {
      ...
      expendGroupApi.getAllExpendGroup().then(res => {
    ...
    deleted (id, name) {
      if(confirm('Xóa ' + name + ". Bạn có chắc không?")) {
        expendGroupApi.deleteExpendGroup(id).then(res => {
          // Reload data
          this.getExpendGroupList()
        }).catch(err => {
          // Handle error
     ...
     */
    edit (id) {
      this.$router.push('/expend-group/index/' + id)
    },
     ...
     */
    goToAdd () {
      this.$router.push('/expend-group/index/')
    }
  }
}
</script>
