    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
      <div class="w-full md:col-span-12">
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
            <div class="w-full md:col-span-12">
              <h4 class="mt-2 text-center">Danh Sách Ví Chi Tiêu</h4>
            </div>
          </div>
          <hr>
...
export default {
   ...
   */
    ...
    getEIBookList () {
      ...
      eiBookApi.getAllEIBook().then(res => {
    ...
    deleted (id, name) {
      if(confirm('Xóa ' + name + ". Bạn có chắc không?")) {
        eiBookApi.deleteEIBook(id).then(res => {
          // Reload data
          this.getEIBookList()
        }).catch(err => {
          // Handle error
     ...
     */
    edit (id) {
      this.$router.push('/ei-book/index/' + id)
    },
     ...
     */
    goToAdd () {
      this.$router.push('/ei-book/index/')
    }
  }
}
</script>
